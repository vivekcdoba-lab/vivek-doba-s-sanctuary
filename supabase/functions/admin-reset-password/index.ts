...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Email via Lovable Emails queue (non-fatal)
    ...
    if (targetProfile.email) {
      ...
      email_sent = r.ok;
      if (!r.ok) email_error = r.error;
    } else {
      email_error = 'Target has no email';
    }

    return json({ success: true, password_updated: true, email_sent, email_error });
  } catch (e) {
