...
import goldenTriangleScoreImage from '@/assets/courses/golden-triangle-score.webp';
import { BUSINESS_PHONE } from './PublicSeo';
  ...
  return <header className={`sticky top-0 z-50 border-b border-border/60 bg-background/95 backdrop-blur-md transition-shadow ${scrolled ? 'shadow-md' : ''}`}>
    <div className="mx-auto hidden max-w-7xl xl:block">
      ...
      <div className="flex h-14 items-center justify-between gap-4 px-4">
        <nav aria-label="Main navigation" className="flex h-full min-w-0 items-stretch">{tabs.map(({ to, label, icon: Icon }) => to === '/courses' ? <div key={to} ref={coursesRef} className="relative flex" onMouseEnter={() => setCoursesOpen(true)} onMouseLeave={() => setCoursesOpen(false)}><NavLink to={to} className={`flex items-center gap-1.5 px-2.5 py-2 text-sm ${isCoursesActive ? 'font-bold text-primary' : 'text-muted-foreground'}`}><Icon className="h-4 w-4" />{label}</NavLink><Button variant="ghost" size="icon" className="h-auto w-7" aria-label="Open Courses menu" aria-expanded={coursesOpen} onClick={() => setCoursesOpen(v => !v)}><ChevronDown className="h-4 w-4" /></Button>{coursesOpen && <div role="menu" className="fixed left-1/2 top-[6.5rem] w-[min(94vw,980px)] -translate-x-1/2 rounded-md border bg-card p-6 shadow-xl"><div className="grid grid-cols-4 gap-5">{courseMenu.map(group => <section key={group.group}><h2 className="mb-2 text-xs font-semibold text-muted-foreground">{group.group}</h2>{group.items.map(item => { const course = courses.find(entry => `/courses/${entry.slug}` === item.href); const imageUrl = course?.cardImageUrl || (item.href === '/score' ? goldenTriangleScoreImage : undefined); return <Link key={item.href} to={item.href} role="menuitem" className="flex gap-3 rounded-md p-2 hover:bg-primary/10">{imageUrl && <img src={imageUrl} alt="" width="1200" height={item.href === '/score' ? 912 : 900} loading="lazy" className="h-11 w-14 rounded object-cover" />}<span><b className="block text-sm">{course?.name || item.label}</b><small className="text-muted-foreground">{course?.duration || item.sub}</small></span></Link>; })}</section>)}</div></div>}</div> : <NavLink key={to} to={to} className={({ isActive }) => `flex items-center gap-1.5 px-2.5 py-2 text-sm ${isActive ? 'font-bold text-primary' : 'text-muted-foreground'}`}><Icon className="h-4 w-4" />{label}</NavLink>)}</nav>
        <div className="flex shrink-0 items-center gap-1.5"><a href={`tel:${BUSINESS_PHONE}`} className="flex items-center text-sm text-muted-foreground"><Phone className="mr-1 h-4 w-4" />{BUSINESS_PHONE}</a><Button variant="ghost" size="sm" onClick={() => openWhatsApp()}><MessageSquare />WhatsApp</Button><Button size="sm" onClick={diagnostic} className="bg-diagnostic text-primary-foreground hover:bg-diagnostic/90">Book a diagnostic</Button></div>
      </div>
    </div>
