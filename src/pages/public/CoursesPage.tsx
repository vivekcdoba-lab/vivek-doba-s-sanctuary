...
import { SITE_URL, allCourses, ladder, sidePrograms, whatsappLink, type Course } from '@/data/courses';
...
export default function PublicCoursesPage() {
  ...
  return <div className="bg-course-ivory">
    ...
    <motion.section {...reveal} className="mx-auto max-w-7xl px-4 py-14"><h2 className="sr-only">Find the program that sounds like you</h2><div className="scrollbar-none flex snap-x gap-4 overflow-x-auto pb-4">{ladder.slice(0,5).map(course => <button key={course.slug} onClick={() => jump(course.slug)} className="min-h-44 min-w-[82vw] snap-start rounded-[18px] border border-primary/15 bg-card p-6 text-left shadow-course transition-colors hover:border-course-saffron sm:min-w-[23rem]"><span className="text-xs font-semibold uppercase text-course-saffron">Step {course.step} · {course.stage}</span><q className="font-course-serif mt-4 block text-2xl italic leading-8 text-course-maroon">{course.hook}</q></button>)}</div></motion.section>
    ...
    <motion.section {...reveal} className="border-y border-primary/15 bg-background px-4 py-16"><div className="mx-auto max-w-6xl"><h2 className="text-3xl font-bold sm:text-4xl">For Sales Teams, Organisations and Readers</h2><div className="mt-8 grid gap-5 lg:grid-cols-3">{sidePrograms.map(course => <article key={course.slug} className="flex flex-col rounded-[18px] border border-primary/15 bg-card p-6 shadow-course"><p className="text-xs font-semibold uppercase text-course-saffron">{course.stage}</p><h3 className="font-course-serif mt-2 text-3xl font-semibold italic">{course.name}</h3><p className="mt-3 flex-1 leading-7 text-muted-foreground">{course.outcome}</p><p className="mt-5 text-sm font-semibold">{course.duration}</p><div className="mt-5"><CoursePrice course={course} /></div><div className="mt-5 flex flex-wrap gap-3"><CourseCta course={course} /><Button asChild variant="link" className="min-h-12"><Link to={`/courses/${course.slug}`}>See details<ArrowRight /></Link></Button></div></article>)}</div></div></motion.section>
