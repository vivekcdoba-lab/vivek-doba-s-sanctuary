...
const AdminAudios = () => {
  ...
  const { data: audios = [], isLoading } = useQuery({
    queryKey: ['learning-content', 'audio'],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">🎵 Audio Content</h1>
          <p className="text-muted-foreground">Manage audio learning resources</p>
        </div>
        <Badge variant="secondary">{audios.length} audios</Badge>
      </div>
      <div className="relative max-w-sm">
        <Search className="absolute left-3 top-3 w-4 h-4 text-muted-foreground" />
        <Input placeholder="Search audios..." className="pl-9" value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      <Card>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Button
                          ...
                          variant="ghost"
                          disabled={!a.url}
                          title={a.url ? 'Play audio' : 'No URL available'}
                          onClick={() => a.url && setPreviewItem({ title: a.title, url: a.url })}
                        >
                          <Play className="w-4 h-4" />
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => toggleActive.mutate({ id: a.id, active: !a.is_active })}>
