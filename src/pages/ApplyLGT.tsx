      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-4">
        ...
        <div className="bg-card rounded-xl p-6 shadow-sm border border-border">
          ...
          <div className="space-y-3 mt-6 border-t border-border pt-4">
            <label className={`flex items-start gap-3 cursor-pointer ${missingFields.has('consent1') ? 'ring-2 ring-destructive/60 rounded-lg p-2 -m-1' : ''}`}><input type="checkbox" checked={f.consent1} onChange={e => set('consent1', e.target.checked)} className="mt-1 rounded" /><span className="text-sm">I declare that all information provided is true and complete. *</span></label>
            <label className={`flex items-start gap-3 cursor-pointer ${missingFields.has('consent2') ? 'ring-2 ring-destructive/60 rounded-lg p-2 -m-1' : ''}`}><input type="checkbox" checked={f.consent2} onChange={e => set('consent2', e.target.checked)} className="mt-1 rounded" /><span className="text-sm">I understand this is an APPLICATION and acceptance is at the discretion of Vivek Doba. *</span></label>
            <label className={`flex items-start gap-3 cursor-pointer ${missingFields.has('consent3') ? 'ring-2 ring-destructive/60 rounded-lg p-2 -m-1' : ''}`}><input type="checkbox" checked={f.consent3} onChange={e => set('consent3', e.target.checked)} className="mt-1 rounded" /><span className="text-sm">I commit to my transformation journey with sincerity, dedication, and discipline. 🙏 *</span></label>
            <label className={`flex items-start gap-3 cursor-pointer ${missingFields.has('consent4') ? 'ring-2 ring-destructive/60 rounded-lg p-2 -m-1' : ''}`}><input type="checkbox" checked={f.consent4} onChange={e => set('consent4', e.target.checked)} className="mt-1 rounded" /><span className="text-sm">I consent to VDTS contacting me via WhatsApp, Email, and Phone. *</span></label>
          </div>
